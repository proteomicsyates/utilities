...
public class TietjenMooreOutlierTest extends AbstractOutlierTest {
	private static double totalTime = 0.0;
	private static int numTests = 0;

	public TietjenMooreOutlierTest(double valueToTest, List<Double> populationValues) {
	...
	public OutlierTestResult test() {
		...
		ArrayList<Object> outlierResult = outlierDetector.outliersTeitjenMoore(2);
		...
		if (!outlierTestResult.isOutlier()) {
			...
			outlierResult = outlierDetector.outliersTeitjenMoore(1);
