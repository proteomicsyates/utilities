...
//			_nrows = 1;
//			final List<String> lines = Files.readAllLines(Paths.get(new File(fileName).toURI()));
//			 
//			_ndims = lines.size();
...
//				final double[] dv = new double[values.size()];
...
//	}
	...
	public void clustering(int numClusters, int niter, double[][] centroids) {
		...
		if (centroids != null)
			...
			final ArrayList idx = new ArrayList();
	...
	private int closest(double[] v) {
		...
		for (int i = 1; i < _numClusters; i++) {
			final double t = dist(v, _centroids[i]);
			if (mindist > t) {
				mindist = t;
				label = i;
			}
	...
	private double[][] updateCentroids() {
		...
		final double[][] newc = new double[_numClusters][]; // new centroids
		final int[] counts = new int[_numClusters]; // sizes of the clusters
		...
		for (int i = 0; i < _nrows; i++) {
			final int cn = _label[i]; // the cluster membership id for record i
			for (int j = 0; j < _ndims; j++) {
	...
	public static void main(String[] astrArgs) {
		 ...
		 * 
		 */
//		final KMeans KM = new KMeans("data.csv", null);
