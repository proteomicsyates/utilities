...
public class PValueCorrection {
	...
	private static int[] order(double[] array, boolean decreasing) {
		...
		final int[] idx = new int[size];
		final double[] baseArr = new double[size];
	...
	public static void doubleSay(double[] array) {
	...
	public static double[] pAdjust(double[] pvalues, PValueCorrectionType method) {
		...
		case HOMMEL:
			...
			break;
		default:
			throw new IllegalArgumentException(method + " doesn't match any accepted FDR types");
		}

		if (type == 2) { // Bonferroni method
			final double[] result = new double[size];
			for (int i = 0; i < size; ++i) {
				final double b = pvalues[i] * size;
				if (b >= 1) {
					result[i] = 1;
				} else if (0 <= b && b < 1) {
					result[i] = b;
			...
			final int[] o = order(pvalues, false);
			final double[] o2Double = intToDouble(o);
			final double[] cummaxInput = new double[size];
			for (int i = 0; i < size; ++i) {
				cummaxInput[i] = (size - i) * pvalues[o[i]];
			}
			final int[] ro = order(o2Double, false);
			final double[] cummaxOutput = cummax(cummaxInput);
			final double[] pmin = pminx(cummaxOutput, 1.0);
			final double[] result = new double[size];
			for (int i = 0; i < size; ++i) {
				result[i] = pmin[ro[i]];
			}
			...
			final int[] indices = seqLen(size, size);
			final int[] o = order(pvalues, false);
			...
			final double[] npi = new double[size];
			for (int i = 0; i < size; ++i) {
				npi[i] = p[i] * size / indices[i];
			}
			final double min = doubleArrayMin(npi);
			Arrays.fill(q, min);
			Arrays.fill(pa, min);
			for (int j = size; j >= 2; --j) {
				final int[] ij = seqLen(1, size - j + 1);
				for (int i = 0; i < size - j + 1; ++i) {
					ij[i]--;
				}
				final int i2Length = j - 1;
				final int[] i2 = new int[i2Length];
				for (int i = 0; i < i2Length; ++i) {
					i2[i] = size - j + 2 + i - 1;
				}
				double q1 = j * p[i2[0]] / 2.0;
				for (int i = 1; i < i2Length; ++i) {
					final double temp_q1 = p[i2[i]] * j / (2.0 + i);
					if (temp_q1 < q1)
						q1 = temp_q1;
				}
				for (int i = 0; i < size - j + 1; ++i) {
		...
		final double[] ni = new double[size];
		final int[] o = order(pvalues, true);
		final double[] oDouble = intToDouble(o);
		for (int i = 0; i < size; ++i) {
		...
		final int[] ro = order(oDouble, false);
		final double[] cumminInput = new double[size];
		if (type == 0) { // BH method
			for (int i = 0; i < size; ++i) {
		...
		final double[] cumminArray = cummin(cumminInput);
		final double[] pmin = pminx(cumminArray, 1.0);
		final double[] result = new double[size];
		for (int i = 0; i < size; ++i) {
			result[i] = pmin[ro[i]];
		}
	...
	public static void main(String[] args) {
		...
		final double[][] correctAnswers = new double[][] { new double[] { // Benjamini-Hochberg
				...
				new double[] { // Benjamini & Yekutieli
						1.000000e+00, 1.000000e+00, 8.940844e-01, 8.510676e-01, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 5.114323e-01, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 1.000000e+00, 1.000000e+00, 1.754486e-01, 1.000000e+00, 6.644618e-01,
						7.575031e-02, 1.153102e-02, 1.581959e-01, 2.812089e-01, 1.636176e-02, 1.153102e-02,
						1.325863e-01, 2.774239e-02, 1.754486e-01, 1.209832e-02, 2.025930e-03, 5.634031e-05,
						3.546073e-01, 1.413926e-01, 2.180552e-02, 1.153102e-02, 2.180552e-02, 4.956812e-03,
				...
				new double[] { // Bonferroni
						...
						1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 1.000000e+00, 1.000000e+00, 7.019185e-01, 1.000000e+00, 1.000000e+00,
						2.020365e-01, 1.516674e-02, 5.625735e-01, 1.000000e+00, 2.909271e-02, 1.537741e-02,
						4.125636e-01, 6.782670e-02, 6.803480e-01, 1.882294e-02, 9.005725e-04, 1.252228e-05,
						1.000000e+00, 4.713920e-01, 4.395577e-02, 1.088915e-02, 4.846527e-02, 3.305125e-03,
						1.000000e+00, 2.867745e-01 },
				new double[] { // Hochberg
						9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.991834e-01, 9.991834e-01, 4.632662e-01, 9.991834e-01, 9.991834e-01,
						1.575885e-01, 1.383967e-02, 3.938014e-01, 7.600230e-01, 2.501973e-02, 1.383967e-02,
						3.052971e-01, 5.426136e-02, 4.626366e-01, 1.656419e-02, 8.825610e-04, 1.252228e-05,
						9.930759e-01, 3.394022e-01, 3.692284e-02, 1.023581e-02, 3.974152e-02, 3.172920e-03,
				...
				new double[] { // Holm
						...
						1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00, 1.000000e+00,
						1.000000e+00, 1.000000e+00, 1.000000e+00, 4.632662e-01, 1.000000e+00, 1.000000e+00,
						1.575885e-01, 1.395341e-02, 3.938014e-01, 7.600230e-01, 2.501973e-02, 1.395341e-02,
						3.052971e-01, 5.426136e-02, 4.626366e-01, 1.656419e-02, 8.825610e-04, 1.252228e-05,
						9.930759e-01, 3.394022e-01, 3.692284e-02, 1.023581e-02, 3.974152e-02, 3.172920e-03,
						8.992520e-01, 2.179486e-01 },
				new double[] { // Hommel
						9.991834e-01, 9.991834e-01, 9.991834e-01, 9.987624e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.595180e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01, 9.991834e-01,
						9.991834e-01, 9.991834e-01, 9.991834e-01, 4.351895e-01, 9.991834e-01, 9.766522e-01,
						1.414256e-01, 1.304340e-02, 3.530937e-01, 6.887709e-01, 2.385602e-02, 1.322457e-02,
						2.722920e-01, 5.426136e-02, 4.218158e-01, 1.581127e-02, 8.825610e-04, 1.252228e-05,
						8.743649e-01, 3.016908e-01, 3.516461e-02, 9.582456e-03, 3.877222e-02, 3.172920e-03,
						8.122276e-01, 1.950067e-01 } };

		final PValueCorrectionType[] types = PValueCorrectionType.values();
		for (int type = 0; type < types.length; ++type) {
			final double[] q = pAdjust(pvalues, types[type]);
			double error = 0.0;
