 ...
 * with the addition of a reduction function at the end that may be executed by
 * any of the threads.
 * <p>
 * Note that the number of thread-local values used in the reduction is
 * determined according to the number of unique <code>Thread</code>s that
 * {@link #set(Object)} a value.
 *
		...
		if (!threadValues.containsKey(tid)) {
			if (initialValue == null)
				throw new RuntimeException(
						"Attempting to get thread-local value of Reducible without first being initialised!");
			else
	...
	public E reduce(Reduction<E> reduction) {
		...
		if (numThreads == 0) {
			...
			if (initialValue == null) {
				reductionLock.unlock();
				throw new RuntimeException(
						"Attempting to perform a reduction with no thread-local values or initial value!");
			} else {
